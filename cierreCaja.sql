...
select T0."DocEntry", T0."CardCode", T0."NumAtCard", T0."CardName", T0."DocDate", T0."TaxDate", 
        case when T0."CANCELED" = 'Y' THEN ((T0."DocTotal")*(-1)) ELSE ((T0."DocTotal")*(-1)) END as DocTotal,
        T0."DocNum",T0."U_NControlFE",'5' Doctype   ,'ANULACIONES-INVALIDACIONES', T0."GroupNum", T1."SeriesName" , T0."UserSign"                   
        ,'0','0','0','0', 'UNAME', CASE WHEN SUBSTRING(T1."SeriesName",5,2) = 'JP' THEN 'JUAN PABLO'
        WHEN SUBSTRING(T1."SeriesName",5,2) = 'ML' THEN 'MERLIOT'
        WHEN SUBSTRING(T1."SeriesName",5,2) = 'SM' THEN 'SAN MIGUEL' ELSE SUBSTRING(T1."SeriesName",5,2) END Sucursal,
        CASE WHEN T0."GroupNum" IN (1,2,4,5,6,7,8) THEN ((T0."DocTotal")*(-1)) ELSE 0 END Credito,
        CASE WHEN T0."GroupNum" IN (3) THEN ((T0."DocTotal")*(-1)) ELSE 0 END Contado, ((T0."VatSum")*(-1)), 'E',
        T0."U_Cod_Sucursal_FEL"
from OINV T0 
...
--where T0."DocDate"={?fechaCierre} and T5."U_NAME"='{?CodUser}'-- and T0."GroupNum" not in ('3')
--and T0."UserSign"='{?CodUser}' 
where T0."DocDate"='2025' and T5."U_NAME"='{?CodUser}'
AND T0."CANCELED" ='Y' 
...
select T0."DocEntry",  T0."CardCode", T0."NumAtCard", T0."CardName", T0."DocDate", T0."TaxDate",
        ...
        WHEN SUBSTRING(T1."SeriesName",5,2) = 'SM' THEN 'SAN MIGUEL' ELSE SUBSTRING(T1."SeriesName",5,2) END Sucursal,
        CASE WHEN T0."GroupNum" IN (1,2,4,5,6,7,8) THEN ((T0."DocTotal")*(-1)) ELSE 0 END Credito,
        CASE WHEN T0."GroupNum" IN (3) THEN ((T0."DocTotal")*(-1)) ELSE 0 END Contado, ((T0."VatSum")*(-1)), 'F',
        T0."U_Cod_Sucursal_FEL"
...
where T0."DocDate"={?fechaCierre} and T5."U_NAME"='{?CodUser}' and T0."Canceled" = 'N' and T3."GroupNum" IN (3) and T0."DocDate" != CURRENT_DATE
...
select T0."DocEntry", T0."CardCode", T0."NumAtCard", T0."CardName", T0."DocDate", T0."TaxDate", 
        ...
        WHEN SUBSTRING(T1."SeriesName",5,2) = 'SM' THEN 'SAN MIGUEL' ELSE SUBSTRING(T1."SeriesName",5,2) END Sucursal,
        CASE WHEN T0."GroupNum" IN (1,2,4,5,6,7,8) THEN ((T0."DocTotal")*(-1)) ELSE 0 END Credito,
        CASE WHEN T0."GroupNum" IN (3) THEN ((T0."DocTotal")*(-1)) ELSE 0 END Contado, T0."VatSum", 'E',
        T0."U_Cod_Sucursal_FEL"
from OINV T0 
...
select T0."DocEntry",  T0."CardCode", T0."NumAtCard", T0."CardName", T0."DocDate", T0."TaxDate",
        ...
        WHEN SUBSTRING(T1."SeriesName",5,2) = 'SM' THEN 'SAN MIGUEL' ELSE SUBSTRING(T1."SeriesName",5,2) END Sucursal,
        CASE WHEN T0."GroupNum" IN (1,2,4,5,6,7,8) THEN ((T0."DocTotal")*(-1)) ELSE 0 END Credito,
        CASE WHEN T0."GroupNum" IN (3) THEN ((T0."DocTotal")*(-1)) ELSE 0 END Contado, T0."VatSum", 'F',
        T0."U_Cod_Sucursal_FEL"
